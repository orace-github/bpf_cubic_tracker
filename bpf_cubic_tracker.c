...
#include "bpf_cubic_tracker.skel.h"
...
int main(int argc, char **argv){
  ...
  struct bpf_cubic_tracker_bpf *skel;
  int err;
  ...
  skel = bpf_cubic_tracker_bpf__open();
  if (!skel) {
  ...
  err = bpf_cubic_tracker_bpf__load(skel);
  if (err) {
  ...
  /* Attach tracepoints */
  err = bpf_cubic_tracker_bpf__attach(skel);
  if (err) {
...
cleanup:
	...
	bpf_cubic_tracker_bpf__destroy(skel);
